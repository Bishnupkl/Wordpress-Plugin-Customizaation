...
if ( !function_exists( 'buddyboss_entry_meta' ) ) :
	...
	function buddyboss_entry_meta( $show_author = true, $show_date = true, $show_comment_info = true ) {
		...
		$date = sprintf( '<a href="%1$s" title="%2$s" rel="bookmark" class="post-date far fa-clock"><time class="entry-date" datetime="%3$s">%4$s</time></a>', esc_url( get_permalink() ), esc_attr( get_the_time() ), esc_attr( get_the_date( 'c' ) ), esc_html( get_the_date() )
		);
		...
		if ( $show_comment_info ) {
			if ( comments_open() ) :
				?>
				<!-- reply link -->
				<span class="comments-link far fa-comment">
...
function buddyboss_bp_messages_options() {
	...
	<a href="#" id="delete_<?php echo bp_current_action(); ?>_messages" class="fas fa-trash-alt"></a> &nbsp;
...
function woo_add_to_cart_ajax( $fragments ) {
	...
	?>
	<a class="cart-notification notification-link fa fa-shopping-cart <?php echo $mobile_classes; ?>" href="<?php echo wc_get_cart_url(); ?>">
		<?php if ( $cart_items ) { ?>
	...
	function boss_cart_icon_html() {
		...
		if ( $woocommerce ) {
			...
			<div class="header-notifications cart">
				<a class="cart-notification notification-link fa fa-shopping-cart" href="<?php echo wc_get_cart_url() ?>">
					<?php if ( $cart_items ) { ?>
						<span><?php echo $cart_items; ?></span>
					<?php } ?>
				</a>
	...
	function boss_mobile_cart_icon_html() {
		...
		if ( $woocommerce ) {
            ...
            <div id="cart-nav-wrap" class="btn-wrap">
                <a id="cart-nav" class="cart-notification fa fa-shopping-cart sidebar-btn table-cell" href="<?php echo wc_get_cart_url() ?>">
                    <?php if ( $cart_items ) { ?>
                        <span><?php echo $cart_items; ?></span>
                    <?php } ?>
                </a>
